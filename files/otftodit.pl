...
sub usage {
	...
	" [-w space-width] [-F opentype-feature] [-V]" .
	" [-o output-file] otf-file [afm-file] map-file" .
...
warn "# $1\n" if $afm =~ /(.*?)\|$/;
open(AFM, $afm) || croak("unable to open '$afm': $!\n");
...
if ($otf) {
    ...
    $notice     //= get_name($otf, 0) // $otf->{'CFF '}->TopDICT->{Notice};
    $version    //= get_name($otf, 5) // $otf->{'CFF '}->TopDICT->{version};
    $fullname   //= get_name($otf, 4) // $otf->{'CFF '}->TopDICT->{FullName};
    $familyname //= get_name($otf, 1) // $otf->{'CFF '}->TopDICT->{FamilyName};
}
...
if ($iscidfont) {
    ...
    for my $k (keys %$liga) {
        ...
        for my $gid ($k, @{$liga->{$k}}) {
            ...
            warn "$prog: $gid => [@glyphname]: not uniq\n" unless @glyphname == 1;
